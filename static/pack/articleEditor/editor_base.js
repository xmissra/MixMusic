...
function renewContent() {
	if (window.confirm('您确定要恢复上次保存?')) {
		var oArea = parent.$('in_intro');
		var f = window.frames["HtmlEditor"];
		try {
			oArea.load('UCHome');
			var oXMLDoc = oArea.XMLDocument;
			oArea.value = oXMLDoc.selectSingleNode("/ROOTSTUB/in_intro").nodeTypedValue;
			f.document.body.innerHTML = oArea.value;
		} catch(e) {
			if (window.sessionStorage) {
				try {
					f.document.body.innerHTML = sessionStorage.getItem('in_intro');
				} catch(e) {}
			}
		}
	}
