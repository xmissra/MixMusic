...
$(document).ready(function(){$("#uploadify").uploadify({uploader:in_path+"static/pack/upload/uploadify.swf",script:in_path+"source/plugin/webim/api/uploadify.php",wmode:"transparent",folder:"UploadFile",method:"GET",scriptData:{uid:in_uid},buttonImg:in_path+"source/plugin/webim/api/uploadify.gif",width:"17",height:"16",queueID:"send_tips",auto:true,multi:false,fileExt:"*.jpg;*.gif;*.png;*.rar;*.zip",fileDesc:"选择*.jpg;*.gif;*.png;*.rar;*.zip文件",sizeLimit:200*1024*1024,onError:function(a,b,c,d){if(d.status==404){$("#send_tips").text("上传异常，请重试！");}else if(d.type==="HTTP"){$("#send_tips").text("error "+d.type+" : "+d.status);}else if(d.type==="File Size"){$("#send_tips").text("上传失败，大小不能超过200MB！");}else{$("#send_tips").text("error "+d.type+" : "+d.text);}
$("#send_tips").fadeOut(in_time,function(){$(this).show().text("按 Enter 键快捷发送");});},onComplete:function(event,queueID,fileObj,response,data){if(response==0){$("#send_tips").text("上传出错，请重试！");}else{$("#textarea").val($("#textarea").val()+response);$("#textarea").focus();$("#send_tips").text("恭喜，文件上传成功！");}
$("#send_tips").fadeOut(in_time,function(){$(this).show().text("按 Enter 键快捷发送");});}});});