...
function Playergeci() {
	...
	this.doSp = function(oT) {
		if (parseInt(oT) > 1) {
			if (isIE) {
				document.getElementById("mediaPlayerObj").controls.currentPosition = oT;
			} else {
	...
	this.PlayLrc = function() {
		...
		if ($song_Lrc[t]) {
			try {
				if (isIE) {
					var curTime = parseInt(document.getElementById("mediaPlayerObj").controls.currentPosition * 1000) + 500;
				} else {
